...
| P2-O18 | Windows shell surface coverage | Add explicit shell coverage for Start menu/search, taskbar, system tray/overflow, Action Center/notification center, Quick Settings, multi-monitor/DPI, virtual desktop detection, UAC/elevated targets, RDP/locked-session behavior, and Explorer-specific file destinations. New commands are added only where a ref-based `snapshot --surface …` loop cannot expose the surface first; Windows-only behavior still routes through core command files and adapter trait defaults | **Shipped by §2.14** (this PR): the shell-surface kinds this build exposes open and snapshot by kind with build-conditional refusals, `list-surfaces` returns real per-process inventories, the tray drives through refs with no bespoke commands, and the four notification commands run over the Action Center — see §2.14's ticked exit criteria. Multi-monitor, RDP/locked-session capture and UAC/elevated launch effects ride §2.15's existing bullets; virtual-desktop detection was dropped at §2.4 (A16-9) |
...
- **Dogfood is a gate, and its findings become tests. In force from §2.11 onward.** Every sub-phase drives its own surface against real software, not only fixtures, and commits a judged report. Three rules make that a gate rather than a ritual:
  ...
  - **The dogfood is run as a stranger to the code, or it is not a dogfood.** The operator is given the shipped skill and the built binary and nothing else — not the source tree, not the plan, not this document. When that operator is a subagent, its brief says so and withholds the repo. This is the rule with the most evidence behind it: §2.15's dogfood, run by the author of the sub-phase, passed; the same surface driven by a consumer holding only the skill produced five defects inside twenty minutes, two of which made the shipped examples impossible to execute at all. The author reached for `--session` and quoted refs by reflex because they knew to. Nobody else knows to.
    - **Every example the shipped documentation contains is executed verbatim, in the platform's default shell.** An example that cannot run as written is a defect of the same weight as a broken command, because it is what a new agent will actually type. §2.15 shipped `@ref` examples that PowerShell — the default shell on Windows — silently deletes before the binary sees them.
    - **Reaching for anything the skill does not document ends the run as a finding.** A flag, an environment variable, a preparatory command or an ordering the operator knows from the source but the skill never states is not a workaround, it is the gap. Author knowledge is exactly what the run exists to remove.
  - **A failure is not a finding until it survives a second, independent observation.** Three of §2.15's reported failures were the harness racing an asynchronous dialog, not the product. Concretely: no step's output may be discarded, every step's `ok` is read, and waiting uses the tool's own wait primitives — a fixed sleep followed by a check is a race, and a failure seen that way is not reportable until a real wait reproduces it.
  - **A behavioural claim is measured in both directions before it is written down.** The failing case and the passing case, both, before the claim reaches a report or a shipped doc. §2.15 recorded that a backgrounded window cannot be driven and put that in the skill; measuring the passing case showed backgrounded windows drive fine and only *minimized* ones fail, so the shipped advice told every agent to steal the user's foreground for nothing. A claim measured once names whichever cause happened to be present.
...
- **Relationship to §2.12 and §2.14.** The `focused_window` frame-versus-`CoreWindow` *identity* question needs a rig with a modern-shell population that neither §2.12's WinForms fixture nor any box measured through it carries (grounding G3, A10-7), so it rides §2.14 rather than §2.12. This sub-phase owns the *tree and resolution* behavior, which needs neither that rig nor a fixture at all — Settings is present and dumpable on the dev box today. If this sub-phase's descent settles the identity mapping as a side effect, §2.14's item narrows to confirming it on its own runner
...
| Notifications | UIA Action Center traversal; `UserNotificationListener` is not consulted (A26-4) | See Notification Management approach under 2.14 |
...
- **Window activation and focus policy in full, over the minimal enabling surface 2.6 shipped.** 2.6 landed `resolve_window_strict` plus a `focus_window` that corroborates ownership on both sides of every native write and qualifies success by live ownership, because the headed path had to reach the occlusion gate. **Element-level `Action::SetFocus` is already settled by §2.7** — headed-only with focused-element verification, `POLICY_DENIED` headless, on A3-4 / A19-5 — and this sub-phase must not re-open that gate. What remains here is the **window-activation** half: restore-versus-raise ordering when the target is minimized behind other windows, a bounded focus-steal budget of 2, confirmed by ownership re-read rather than trusted from the API's return value (A21-6 measured the uncontended rate at 0/5 for both the first and second attempt; §2.12 re-measured it contended, from two fixture windows staged under its `DesktopLease` → `ForegroundStage` ordering, and recorded the result as a probe row beside it), cross-virtual-desktop behaviour as attempt-and-verify-ownership (no `IVirtualDesktopManager` binding, A16-9), and the UIPI boundary — integrity comparison up front, with an ownership-qualified foreground re-read meant to catch a silent no-op as activation-worded `PERM_DENIED`/`not_delivered`. **§2.12's live confirmation measured this fail-closed claim false for `focus-window`:** against a genuinely staged Medium-integrity caller and a High-integrity fixture target, with a foreground precondition that rules out a no-op success, `focus-window` returns `ok:true` and a raw `GetForegroundWindow` re-read confirms the target genuinely became foreground (`tests/e2e-windows/scenarios/SplitIntegrity.ps1`, `Invoke-SplitIntegrityActivationLeg`). `SetForegroundWindow` is gated on Windows by the foreground-lock-timeout heuristic (recent input, `AllowSetForegroundWindow`), not by Mandatory Integrity Control the way `SendInput`/`PostMessage` are — UIPI does not cover this API, so the integrity comparison this bullet describes does not stop the activation it names. §2.15 owns deciding and shipping the fix, as part of its whole-branch hardening review. The residual TOCTOU on a recycled HWND is irreducible at the Win32 API level; 2.6 made losing that race fail closed, and this sub-phase confirms nothing stronger is warranted
...
**Scope:**
- **The Windows cross-process interaction lease.** `crates/core/src/interaction_lease.rs` is `#[cfg(unix)]` in its entirety and `WindowsAdapter::acquire_interaction_lease` holds no lock (`A24-10`), so the harness this sub-phase ships has no exclusivity primitive to stand on. The lease lands in `crates/windows/src/system/`: an exclusive `CreateFileW` open of a token-derived canonical path resolved without the reserved `Win32_UI_Shell` feature, adoptable through an inherited handle whose `(volume serial, file index)` identity **and** exclusivity are both re-verified, `TIMEOUT` on contention, `POLICY_DENIED` on identity mismatch or a non-exclusive handle. This changes shipped behavior: two concurrent `agent-desktop` invocations that both need a lease now contend where previously neither did
- WinForms fixture app compiled with `csc.exe` (.NET 4.8 preinstalled on the runner — no new toolchain dependency)
- `AutomationId` set on every interactive target from day one (unlike macOS, which had to retrofit `AXIdentifier` — Windows gets this right from the start). The mechanism is WinForms `Control.Name`, which the stock provider surfaces as UIA `AutomationId`; no `.exe.config` is required to expose it, and the `Switch.UseLegacyAccessibilityFeatures*` overrides make no difference to it either way (A24-2, measured across both arms). Two shapes the fixture author has to design against rather than discover: a `TextBox` presents as `ControlType.Edit`, not `TextBox`, so a role expectation written from the control's type name is wrong; and the stock provider emits `TitleBar`, `MenuBar`, `MenuItem` and the caption buttons beside the author's own controls, so an assertion over a fixture window's children must select rather than count. The in-box compiler is the legacy pre-Roslyn `csc.exe` and accepts **C# 5** at most — `/langversion:6` and above fail `CS1617` — so the fixture is written without string interpolation, expression-bodied members, `nameof` or null-conditional operators (A24-1)
- Fixture targets: delayed-enable, zero-bounds, duplicate-title and disclosure mirror `AgentDeskFixture.swift`. Occlusion does not — a grep over `tests/fixture-app/*.swift` and every file under `tests/e2e/` finds no occlusion element and no occlusion scenario there, so the Windows occlusion class is designed fresh against this crate's own named-overlay staging (`crates/windows/src/tree/fixture_overlay.rs`), whose overlay is named so a hit-test assertion can prove which window intercepted it. The disclosure class is asserted behaviourally rather than by role: the Windows role map's `button_role` (`crates/windows/src/tree/roles.rs`) has no `disclosure` arm — a `Button` advertising `ExpandCollapse` refines to `menubutton`, one advertising `Toggle` refines to `switch` — so the fixture proves disclosure through `expand`/`collapse` against a `treeitem` and a `menubutton` with the `expanded` state read back through `get`, a stronger oracle than the macOS fixture's child-presence check, where `DisclosureGroup` exposes no state at all
...
- Self-hosted interactive Windows runner registration is **declined here**, on the owner's decision (§Key Decisions): registering a self-hosted runner on a public repository is an outward-facing security action the repository owner reserves, and GitHub's own guidance advises against self-hosted runners on public repositories at all, because any fork PR can otherwise execute untrusted code on a machine the owner controls. What ships here instead: the registration artifacts — `windows-e2e.yml`, its trigger policy, the fork-PR approval policy, the ephemeral-versus-persistent decision, `scripts/run-windows-e2e-ci.ps1` with its refusal guard, and `docs/runbooks/windows-self-hosted-runner.md` — written down and reviewable with no runner to execute them, plus the whole live suite run locally under the new `DesktopLease` (above) on this box, which is this sub-phase's own gate. Registration was declined a second time at §2.15, and the live e2e lane was **removed from CI entirely** on the owner's decision: the runner's labels are reachable from every `pull_request`-triggered workflow in the repository, not from one file, so a fork PR editing any of them would be code execution on the owner's interactive desktop. The live Windows suite is run locally on demand with `scripts/run-windows-e2e-ci.ps1` under the exclusive `DesktopLease`, which is where every scenario in it has actually been verified; CI runs the Windows unit tests, clippy, the contract gates and the harness self-tests on GitHub-hosted `windows-latest` and `windows-11-arm`, none of which needs a desktop. `windows-e2e.yml` and the static assertion that policed its capture upload are deleted rather than left committed and unclaimed; `docs/runbooks/windows-self-hosted-runner.md` is kept unexecuted so a future registration has a reviewable policy to register against. The CI-green obligation that rode §2.15 — the sub-phase that already owes a live end-to-end run on a Windows runner as the branch-final gate before promotion to `main`, and that already receives the multi-monitor and session-degradation rig decisions this same runner would provision. A service-mode runner has no interactive desktop and cannot see UIA at all, so whenever it registers, the runner must launch from a Task Scheduler task triggered at log-on that runs `run.cmd` inside the interactive session
- Public-repo hardening on that registration. This repository is public, and GitHub's own guidance is that self-hosted runners should almost never be used for public repositories, because any user can open a pull request and compromise the environment: the runner's workflow is triggered only by `workflow_dispatch` and by `push` to `feat/windows-adapter`, never by `pull_request` — GitHub offers a `workflow_dispatch` run only for a workflow file present on the default branch, and no Windows sub-phase lands there, so dispatch alone would never be offered; a `push` trigger runs at the pushed ref and cannot be fired from a fork. The fork-PR approval policy is written down, and ephemeral/JIT versus persistent registration is an explicit recorded decision rather than a default. §2.15 re-ratifies this trigger policy at the point it actually registers a runner, since the branch-scoped `push` grants no live exposure until a runner claims the queued job
- Registration remains a measurement obligation as much as infrastructure — it would create the first non-console session this project can observe (an interactive session is required for UIA to see a real desktop; `tscon` is the documented console-reattach workaround and leaves the machine unlocked — see Risk Register) — but the owner declined to register a runner in this sub-phase (§Key Decisions), so that measurement does not land here. 2.0's deferred RDP/session-isolation row (`A10-2`) rides §2.15 alongside registration itself. Until it lands, no Windows adapter behavior assumes console-session semantics and this document claims no RDP or remote-session support
...
- `windows-e2e` workflow_dispatch job written and shipped, targeting the `[self-hosted, Windows, agent-desktop-e2e]` labels §2.15 allocates once it registers a runner to claim it
- **`focused_window`'s frame-vs-`CoreWindow` identity for `ApplicationFrameHost`-hosted targets.** A16-2 could not settle which HWND shape a UWP host presents to `GetForegroundWindow` and `list_windows`, and the ground recorded then — no environment carries a modern-shell population (A10-7) — was narrower than written: A10-7's measured statement is the absence of a **WinUI3/MSIX** population, and this question concerns the UWP `CoreWindow` shape this box presents, which A1-3 had already split statically. §2.14 measured the missing foreground reading on this box: with Settings frontmost, `GetForegroundWindow` returns the `ApplicationFrameWindow` owned by `ApplicationFrameHost`, whose `ApplicationFrameTitleBarWindow` and `ApplicationFrameInputSinkWindow` children share the frame's pid while the `Windows.UI.Core.CoreWindow` child is owned by the hosted application (A26-8). Settings is single-instanced here — a direct spawn of `SystemSettings.exe` exits silently with no frame, and only the `ms-settings:` route materializes one
- **Split-integrity verification for observation reads, input writes, window activation/focus, *and* capture.** A16-12 measured a Medium-integrity client reading a High-integrity WPF window's root and process-generation token with neither read denied — but the probe's High and Medium processes share one user on the dev box, so KTD3's fail-closed elevated-process branch (`process_instance: None` on a failed token read) never fired there. A rig with a genuinely split-integrity boundary — a Medium-integrity session observing a window an elevated, Administrator-token process owns — does **not** require the runner registration, and the verification lands in this sub-phase's ordinary work. `Start-MediumIntegrityProcess` is one route among several, and its privilege failure was mistaken for the platform's answer; A24-4 measured four routes and staged the documented Medium level on an ordinary dev box with `CreateRestrictedToken` plus `SetTokenInformation(TokenIntegrityLevel)` (`S-1-16-8192`, confirmed by reading the live process token back), and a Low boundary additionally with an `icacls` mandatory label. Every route is judged by that token read-back rather than by its launcher's exit code, because `runas /trustlevel:0x20000` launches successfully and yields a process that is still High. **The write half lands here too:** A9-2 measured that across a UIPI boundary reads cross while `SendInput` silently does not land, and A9-3 measured `SendInput` reporting success in both arms, so only an independent re-read separates them — but neither §2.7 nor §2.8 could stage the boundary to observe the effect, because `Start-MediumIntegrityProcess` fails with "A required privilege is not held by the client" on both probe hosts (A19-4, and A18-4 before it). §2.8 therefore ships and unit-tests UIPI *detection* (a local `GetTokenInformation(TokenIntegrityLevel)` read plus the integrity comparison) and maps the denial onto `PERM_DENIED` from A9-2's already-measured contract; what stays open is the live confirmation that a Medium→High pattern write or `SendInput` really does not land and really does surface as that envelope. The `A24-4`-staged rig is the first one able to hold both integrity levels with no runner, so this sub-phase owns that confirmation — not a re-implementation of the detection, which is already shipped and pinned. **The activation half lands here too:** §2.9's window-activation policy reaches `SetForegroundWindow`/`AttachThreadInput` across the same boundary, and A9-2's "reads cross, writes do not" makes a Medium→High activation a *plausible* silent no-op — §2.9 unit-tests the integrity comparison against synthetic SIDs but could not stage the live effect on any probe host to date (the same `Start-MediumIntegrityProcess` gate), so this sub-phase owns confirming what the live effect actually is (A21-7 deferred exactly that instrumentation here). **Measured, it is not a no-op (A24-16):** staged against a genuine Medium-integrity caller with a foreground precondition ruling out a trivial success, `focus-window` against the High-integrity fixture returns `ok:true` and a raw `GetForegroundWindow` re-read confirms the target genuinely took foreground (`tests/e2e-windows/scenarios/SplitIntegrity.ps1`, `Invoke-SplitIntegrityActivationLeg`) — `SetForegroundWindow`'s own gating (foreground-lock-timeout, `AllowSetForegroundWindow`) is independent of Mandatory Integrity Control, unlike `SendInput`/`PostMessage`, which UIPI does block. §2.9's stated fail-closed claim for this API does not hold; §2.15 owns deciding and shipping the fix as part of its whole-branch hardening review. **The capture half lands here too:** A22-7 manufactured a Medium-integrity caller on the 2.10 host but could not run `PrintWindow`/WGC inside that process to observe the Medium→High capture effect (`printwindow.cross_direction.branch: medium_printwindow_launch_failed`; WGC `supported_cross_direction_not_instrumented`), so live cross-integrity capture effect — Legacy and modern where supported — is owned here rather than re-implemented in 2.10
- **Multi-monitor `list_displays` verification and per-display capture.** A10-3 and A16-4 both measured `list_displays` and per-monitor `scale` against the single 96-DPI display present in every environment measured to date, so the aware-versus-unaware bounds delta those rows report as zero is a fact about that display, not yet about the DPI-scaling code path. A18-9's mixed-DPI verification rests on the same gap, pre-committed unmeasurable for the same reason (zero scale steps on any display measured to date). A22-8 measured that a software/virtual second display cannot be manufactured on this class of box (`manufacturable: false`, `branch: no_virtual_display_adapter_available`), so per-display `screenshot --screen <index>` correctness joins the multi-monitor item rather than closing inside 2.10. A rig with more than one monitor, ideally at mismatched DPI, closes all three legs; this box exposes exactly one display (grounding G2) and no runner is registered at all (§Key Decisions), so §2.15 provisions or ratifies a multi-monitor rig as part of its whole-branch review
- **Capture session-degradation.** 2.10 proves silent Modern→Legacy fallback at the forced-unavailable seam and on the 17763 dogfood host where interop fails despite `IsSupported` (A22-1; dogfood J0); that evidence stands and closes here. Genuine RDP / locked-desktop / Session-0 degradation — a session someone can disconnect or lock — needs a second, disposable interactive host: this session cannot be unlocked from inside itself, and a failed reattach strands the box, so producing the condition on the one box available here is a policy hazard rather than a measurement this sub-phase may take. It rides §2.15 together with multi-monitor verification, as one provision-or-ratify decision. Live modern-capture pixel success is **not** an unconditional obligation of this item: the capability-probe lane on `windows-latest` still owes the hosted-runner WGC reading (A22-1); when that lane proves modern pixels, the proof stays with 2.10's evidence; when that lane finds WGC unsupported or interop-incapable, live modern success attaches here beside session-degradation
...
- **Chromium / Electron semantic-action proof on a settled desktop.** §2.7's dogfood drove one Obsidian semantic click and recorded honest `TIMEOUT` / `not_delivered` (J9); the same host's shell-bound Chromium shape is A18-3. Neither invents semantic success. This sub-phase's controlled, vault-configured Chromium target — no runner needed — owns proving a positive-area leaf through the semantic dispatch tier without coercing the environment-bound failure into a pass
- **Explorer below-fold / Items View virtualization surface for the ancestor-scroll ladder.** §2.7 shipped the `ScrollPattern` ancestor ladder (A19-7) and re-judged the 2.6 Explorer residual as dispatch-honest Invoke with `delivered_unverified` (2.7 dogfood J3). On that host the realized option set had no off-screen row among virtualized Items View children (24 of 40 synthetic files visible), so the ladder was not forced through a below-fold geometry change. This sub-phase owns a fixture surface that forces the ladder — not a re-implementation of the ladder itself, and not a denser Explorer, which A24-7 measured shut: at 40, 200 and 1000 staged files in one small window, Explorer realized exactly the eight children that were visible and none below the fold in every leg, so a twenty-five-fold density increase moves the realized count by zero. Unrealized rows are not UIA children at all, which is why density cannot reach them
- **The HWND uniqueness-counter wrap rate under real window churn.** `A24-8` measured it in-process against a scratch window class, with no runner: 91,923 windows created in a 90-second wall-clock cap, 65,534 distinct full HWND values, first full-value reuse at creation 65,535, 26,389 reuse events after the wrap, over a 16-bit generation field shared desktop-wide across the handle table. The counter wraps and full-value handle reuse is reachable, so §2.12.1 ships both the per-window immutable identity field on `RefEntry` **and** the wrap-handling rule beside it, not the field alone
- **Menu-open detection for the stacks area 23 could not stage.** §2.11's detector is two measured sources whose union covers Win32 and WPF, and both were verified round-tripping false-to-true-to-false. Two stacks stayed unmeasured and are recorded `measurable: false` rather than assumed: **Chromium/Electron** (A23-3 — no menu surface was reachable by generic staging against the installed build across three real-input attempts, so neither source could be evaluated), and **WinUI3/MSIX** (no WinUI3/MSIX host exists in any environment measured to date — A10-7's measured claim read precisely; a UWP `CoreWindow` host *is* presented by this box's sessions and was reached and evaluated at rest, though no generic staging raised a menu in it (A26-11, A26-8), and a Chromium host was likewise presented and evaluated (A26-12)). `A24-6` reconfirmed A23-3 independently — two further real-input staging attempts against the same build, activation confirmed first, no source fired and no false positive at rest — so this sub-phase makes one more, vault-configured attempt at the Chromium leg plus a host-wide search for any other reachable Chromium/Electron shell, and records the result rather than re-implementing the detector. The WinUI leg has no fixture route at all — WinForms cannot manufacture an `ApplicationFrameHost` target — and rode §2.14, which reached and evaluated the UWP host at rest and hands the unevaluated menu-open half to §2.15 with the staging record attached (A26-11)
- **The mid-walk identity race rate under a standard-user integrity level.** A23-5 measured 0 hits in 120 iterations of the agent-facing window enumeration on this host, idle and under churn, at High-integrity Administrator with up to 286 windows per iteration. That is not a contradiction of the race being reachable — §2.11 observed it firing directly in the launch path and ships a bounded re-walk for it — but it is a rate this host could not reproduce at High integrity, so the budget of 5 attempts rested on the sighting rather than on a measured distribution. `A24-4` staged a genuine Medium-integrity observer on this box with no runner (`CreateRestrictedToken` plus `SetTokenInformation(TokenIntegrityLevel)`, confirmed `S-1-16-8192` by token read-back), so this sub-phase runs §2.11's window-enumeration race loop from that staged observer for a stated iteration count and records the hit rate as a probe row, resting the budget on a distribution rather than on one sighting
- **Whether a window this process can never identify is a real population.** §2.11 accepts that a window whose owning-process identity is unreadable is excluded from the signal inventory and therefore permanently invisible to the diff — a `window-opened` for it will not fire. That is the honest limit of what a Medium-integrity observer can see, and A23-9 measured 0 such windows on this desktop at High integrity, which is a floor rather than a general answer. This sub-phase runs the agent-facing window enumeration from the same `A24-4`-staged Medium observer used above and records the count of windows whose owning-process identity is unreadable from that vantage — the bound A23-9's High-integrity floor could not take. The residual difference between a `CreateRestrictedToken` observer and a standard-user logon session is **accepted**: the UIPI decisions both measurements bear on key on the integrity level a token carries, which is what is measured directly by read-back, and any behaviour that depends on the logon session rather than the integrity level is outside what either measurement claims
- **The `cell` ref-able role arm.** A16-10 and the 2.4 dogfood report (`docs/dogfood-reports/2026-08-01-feat-windows-2-4-observation-dogfood.md`) measured that neither fixture available to 2.4 produces the `DataItem` + `GridItem`/`TableItem` shape the `cell` refinement needs: WPF's `DataGrid` exposes the patterns on `ControlType.Custom` cells instead, and WinForms' `DataGridView` exposes neither pattern on its rows. This sub-phase's fixture app is where a grid target that resolves the question belongs — either reproducing the `Custom`+pattern shape against a fixture the harness controls, or observed against a real grid application — settling what the dogfood report left open: whether `Custom` + `GridItem`/`TableItem` should also refine to `cell`. `A24-9` proved the route works under the pinned toolchain: a custom UIA provider compiled with `csc.exe` 4.8 forces the `DataItem` + `GridItem`/`TableItem` shape and a managed UIA client reads both patterns back. This sub-phase therefore decides the `Custom` + `GridItem`/`TableItem` refinement rather than recording the shape as still open, and re-measures stock `DataGridView` against a live grid rather than resting on 2.4's reading alone (`A24-9` left `stock_dgv.measurable: false`, `branch: dgvRows_element_not_found`)

**Key APIs:** `csc.exe`, WinForms `Control.Name` surfaced as UIA `AutomationId` (A24-3 — `AutomationProperties` is a WPF attached property and has no WinForms equivalent)

**Depends on:** 2.7, 2.8, 2.9, 2.10, 2.11 (everything the harness exercises)

**Exit criteria:** a second concurrent desktop consumer is refused by the new cross-process interaction lease, and every lease acquisition proves both that its handle names the canonical lock and that the lock is held exclusively; split-integrity verification covers observation reads, input writes, window activation/focus, **and** the live Medium→High capture effect (A22-7), all four judged by token read-back on a `CreateRestrictedToken`-staged Medium/Low boundary (`A24-4`); the mid-walk identity race rate and the never-identifiable-window population are each measured from that staged Medium observer over a stated iteration count and recorded as `probes/windows/FINDINGS.md` rows, with the residual gap to a standard-user logon session accepted on the reason stated above; the contended focus-steal rate re-measures A21-6's uncontended 0/5 capture from two fixture windows under the `DesktopLease` → `ForegroundStage` ordering and is recorded beside it; the HWND uniqueness-counter wrap rate is discharged by `A24-8`'s measurement rather than owed here; the graded fallback's `STALE_REF` rate is measured against a target with content actually staged; and the full Windows live gate is green in both headless and headed tiers under the local `DesktopLease`-exclusive run on a developer box — this sub-phase's gate, since the owner declined self-hosted runner registration (§Key Decisions). `windows-e2e.yml`'s trigger policy (`workflow_dispatch` and branch-scoped `push`, never `pull_request`), its fork-PR approval policy and its ephemeral-versus-persistent decision are written down, unclaimed by any runner. **Not** exit criteria here: CI-green-on-a-self-hosted-runner, which is **retired rather than deferred** - the owner declined registration and directed the live e2e lane's removal from CI, so no sub-phase receives it and the live suite's evidence comes from local exclusive runs. Riding §2.15 instead: 2.0's RDP/session-isolation row (`A10-2`) and genuine capture session-degradation; multi-monitor `list_displays` verification and per-display capture. Live modern-capture pixel success is an exit criterion here only when the `windows-latest` capability-probe lane has not already proven modern pixels for 2.10 (A22-1).

**Est. PR size:** ~14.6k LOC as built (measured against the integration branch; the plan estimated ~4.9k and records where each row missed). `docs/plans/2026-08-16-001-feat-windows-2-12-fixture-e2e-harness-plan.md` sizes the actual build at roughly double the original estimate and states why: the harness needs its own cross-process exclusivity primitive Windows has never had (no `#[cfg(windows)]` counterpart existed anywhere in the tree before this sub-phase), a structural contract gate over the harness itself, and five measurement legs the original estimate did not anticipate — split-integrity effects, a content-staged `STALE_REF` rate, a forced ancestor-scroll ladder, the `cell` role arm and the HWND wrap rig. The owner took the single-PR decision explicitly rather than splitting the measurement units into their own PRs, so the overage is sanctioned here rather than left as an unreviewed accident
...
- **A wrap-handling rule ships beside the field.** `A24-8` measured the counter reachable under realistic churn — 65,534 distinct full HWND values before the first reuse, reuse continuing every ~65,535 creations thereafter on a 16-bit generation field shared desktop-wide — so this sub-phase ships the wrap-handling rule that keeps the stored identity honest across a counter wrap, not the field alone
...
**Depends on:** 2.12 (`A24-8`'s measured wrap-rate row, which this sub-phase's wrap-handling rule is built against), 2.5 (the resolver and stored-evidence window check this corroborates)
...
**Scope:**
- FFI real-window coverage on Windows: the real `WindowsAdapter` behind the C ABI drives an observation-to-action round trip against a window the test itself stages (`crates/ffi/tests/c_abi_windows_live_round_trip.rs` + `crates/ffi/tests/common/win32_fixture.rs`, click confirmed by the fixture's own counter, never by the envelope's `ok`). This closes the one gap the packaging review found: the crate's lib tests and envelope-shape integration tests already ran against the real adapter on `windows-latest` (`ci.yml`'s `cargo test --locked -p agent-desktop-ffi --tests` builds both the integration binaries and the lib unittest target; `stub-adapter` is not a default feature), while the stub passthrough job runs on `ubuntu-latest` only — neither half ever touched a real window through `extern "C"`
- npm platform mapping exists once (`npm/lib/platform.js`) instead of three hand-maintained copies; postinstall gains released `win32-x64`/`win32-arm64` entries — hosted `windows-11-arm` runners have been GA for public repos since 2025-08-07, so ARM64 validation is no longer deferred
- Release matrix: the Windows CLI asset is a **`.tar.gz`** containing exactly `agent-desktop.exe` (in-box bsdtar 3.5.2 handles it — A25-1/A25-2), built natively on `windows-latest` / `windows-11-arm`; the Windows **FFI** archive remains a `.zip`, now carrying the MSVC import library (`agent_desktop_ffi.dll.lib`, ~26 KB, A25-6) beside the DLL, plus the ARM64 cdylib leg. Same sha256 + Sigstore pipeline Phase 1.5 ships. `publish-github` refuses a tag that already carries assets unless dispatched with `allow_republish`, so a re-dispatch cannot replace shipped downloads behind green gates
- `skills/agent-desktop-windows/SKILL.md` — see Skill Update below
- README platform table: Windows column states what is true — six groups work, with `list-surfaces` and `launch`-by-display-name caveated, Notifications still Planned
...
**Exit criteria:** restated in the form a sub-phase branch can falsify, because `npm publish` and `gh release create` fire only from `main`: a `workflow_dispatch` run of `release-dry-run.yml` on this branch produces the real Windows assets from this branch's commit and can reach no publish job at all — the dry run lives in its own dispatch-only, `contents: read` workflow because a workflow that both checks out a dispatch-chosen ref and runs on the default branch is a cache-poisoning vector (CodeQL `actions/cache-poisoning/poisonable-step`), which no runtime condition removes, and the npm package installs from those artifacts on a Windows machine (`validateArchive`/`installArchive` against the downloaded archive, then `agent-desktop version`, a live snapshot, and one interaction through the packed npm shim). What stays unproven until Phase 2 close is only GitHub-Release *hosting* of the assets, whose Windows-specific content is zero — `publish-github` globs `*.tar.gz` / `*.zip` and is target-agnostic. `check-npm-package.js` fails when the npm platform table and the release matrices disagree about targets, tarball names, or asset count.
...
- **`list-surfaces` has no Windows override and returns core's `PLATFORM_NOT_SUPPORTED` default today.** Windows advertises `[Window, Focused, Sheet]` for `snapshot --surface` (§2.11) but implements none of `ObservationOps::list_surfaces`; the README and the shipped Windows skill document this honestly rather than flipping the row. Extending per-process surface enumeration to Windows is this sub-phase's work, beside the shell surfaces below — same class of capability, same consumers.
- **Launching an installed app by display name or AUMID, which §2.9's `launch_app` cannot do.** §2.9 launches via `CreateProcessW` with system-dirs-only bare-name resolution (`System32` / Windows directory — never caller directory, current directory, or `PATH`; A21-1), so it never reads the `App Paths` registry key or Start Menu entries; macOS resolves any installed app through `NSWorkspace` by bundle id or display name. Reaching parity needs `ShellExecuteExW` (a `Win32_UI_Shell` manifest feature) or `IApplicationActivationManager` for packaged/UWP apps (an interface the pinned `windows`/`windows-sys` crates do not generate, the same class of gap as `IVirtualDesktopManager` at line 1146). **A21-8** validated `ShellExecuteExW` binding and launch in a standalone scratch that requires `Win32_UI_Shell` **and** `Win32_System_Registry`, recorded `expand_2_9: false`, and named ownership `section_2_14_owns_by_name_aumid` — the positive binding does not pull the capability back into §2.9. §2.9 deliberately did not take that dependency — the manifest surface is a supply-chain-reviewed decision rather than a probe outcome, and `CreateProcessW` is load-bearing for its launch verification design. The decision is recorded here, and it is **not** to take the capability: `ShellExecuteExW` needs `Win32_UI_Shell` **and** `Win32_System_Registry`, and `scripts/check-win32-ui-shell-exclusion.ps1` — which scans both the manifest text and the resolved feature graph — stays in force to keep both out; and a hand-declared `IApplicationActivationManager` was rejected because it reaches only packaged AUMIDs, so it would not close the display-name parity gap that motivated it (A21-8). By-name launch appears in no §2.14 exit criterion, and this document pre-authorizes the recorded-decision branch. Windows `launch` stays path-or-system-image through Phase 2; §2.15 decides only whether the portable `launch` contract normalizes or ratifies that
...
- **`focused_window`'s frame-vs-`CoreWindow` identity for `ApplicationFrameHost`-hosted targets, and the WinUI arm of §2.11's two-source menu detector.** §2.12 could not reach either: it ships a WinForms fixture, which cannot manufacture an `ApplicationFrameHost`-hosted target at all. The box in fact presents a UWP host — §2.14 measured it directly with no runner registered (A26-8), and A10-7's claim read precisely is the absence of a WinUI3/MSIX population, not of a UWP one — so both measurements were taken on this box rather than on a provisioned runner. The frame identity is measured and shipped (A26-8); the detector's WinUI arm was evaluated at rest on the activation-confirmed UWP host, and the unevaluated menu-open half is written into §2.15's scope with the staging attempted and the population searched (A26-11). **Neither took the `PLATFORM_NOT_SUPPORTED` hedge this sub-phase's own shell surfaces are allowed** — both govern every UWP-hosted application an agent might drive, not only this sub-phase's shell-surface scope, so a hedge here would leave a load-bearing platform question unanswered rather than an optional surface unsupported. **Evidence:** A16-2, A1-3, A10-7, A23-4, A26-8, A26-11

- **The Chromium/Electron arm of §2.11's menu detector — evaluated.** §2.12's third, vault-configured staging attempt found no reachable menu surface, and its host-wide search reported no other Chromium/Electron shell present — on a needle list that missed `cursor`. The fourth attempt's corrected population scan (eight image-name needles including `cursor`) matched only `cursor` — sixteen running images of a VS Code fork and therefore Chromium/Electron — against a live, activation-confirmed Cursor window. A content-area right-click staged a real menu (the window's subtree gained two `Menu` elements and one `MenuItem`, positive-area, gone after the ESC restore); both shipped detector sources stayed silent — the Electron context menu is a DOM menu inside the application's own non-tool window, framework `Chrome`, with no native menu tracking — so the detector gained a third, framework-gated source (the pid's visible non-tool uncloaked windows searched for a Chromium-framework `Menu`/`MenuItem`), which fires exactly under the staged menu and reads false at rest and after restore (A26-12). A24-12's population search missed `cursor`: its needle list did not include it. **Evidence:** A23-3, A24-6, A24-12, A26-12
- **A resolve-identity fallback for Chromium content leaves with no accessible name, or an explicit ratification that they stay unclickable through the semantic tier.** §2.12 measured a real, reproducible identity gap rather than an environment artifact: a CommonMark task-list checkbox renders with no associated `<label>`, so it carries neither a `native_id` nor a stable text identity, `crates/windows/src/tree/resolve_search.rs::entry_is_unverifiable` (fed by `has_meaningful_identity`) fails the ref closed as `STALE_REF` before any candidate search runs, and no other `Click`-bearing, named element exists in the same content (every named clickable nearby - `Navigate back/forward`, `More options` - is editor chrome, not content). §2.12's content-staged rate leg measured this concretely: `stale_rate: 0.75` (N=8) against refs from real, threshold-clearing content (`A24-11`). This is adapter identity-composition work shared with every other role and platform, not fixture/harness work, so it is out of §2.12's charter; §2.12's own pre-committed disposition (a same-PR fix or a named deferral) resolves to this bullet. What must land here: either a positive-area geometry/role-position fallback identity for a zero-identity content leaf (extending `resolve_search.rs::provisional_geometry_candidate`'s existing bounds-hash tie-break, which today only arbitrates between multiple candidates, to also admit a *single* geometrically-plausible zero-identity candidate), or an explicit ratified decision that such content is unclickable through the semantic tier by design. **Evidence:** A24-11

**Key APIs:** see the three subsections immediately below.

**Depends on:** 2.4 (observation), 2.7 (semantic actions), 2.9 (`launch_app`, whose by-name/AUMID gap this sub-phase inherits), 2.11 (the menu detector and the surface-signal path this sub-phase extends), 2.12 (the fixture app and live e2e harness this sub-phase's verification builds on)

**Exit criteria:** ticked against what shipped. `open-system-surface --surface <kind>` + `snapshot --surface <kind>` round-trips for the shell kinds this build exposes — Start menu, taskbar, system tray, system-tray overflow, and Action Center — with explicit `PLATFORM_NOT_SUPPORTED` assertions (clear `platform_detail`) where it does not, and `list-surfaces` advertising the shipped kinds: satisfied — the immersive family is reached through the UIA root's children matched by native handle rather than by `EnumWindows`, and its open/closed state reads from `DWMWA_CLOAKED` plus root membership because a dismissed surface's window survives cloaked (A26-1, A26-2); the Start-menu accelerator raises a search-hosted `CoreWindow` rooted at `PaneRoot`/`SearchTextBox` rather than a tile surface (A26-9); and `quick-settings` resolves `PLATFORM_NOT_SUPPORTED` naming `action-center`, where the quick actions are a pane (A26-3, C-10). Notification list/dismiss/action work through the documented path — Action Center traversal over the `MainListView` + `ClearAllButton` landmarks, per-entry `DismissButton` invokes, every mutation verified against the entries it targeted (A26-3); the dogfood drove the mutations live and verified them by re-read: a sole-entry dismiss confirmed removed, `dismiss-all` reporting its captured-set outcome (`dismissed_count` equal to the captured members verified gone), an unknown action refused `ACTION_NOT_SUPPORTED` with the entry unchanged, and an expired entry refused `NOTIFICATION_NOT_FOUND` rather than dismissed. Tray refs resolve through the generic surface: `snapshot --surface system-tray-overflow` allocates refs to the five overflow `Button` items, and the promoted toolbar's three `Button`s (stable machine-local `AutomationId`s, `Invoke`, positive-area bounds) surface through `snapshot --surface taskbar` (A26-5, A26-7) — but the dogfood did not bear out a working tray click: the dedicated `system-tray` surface's promoted read returned zero items while the taskbar path ref'd the same toolbar, the overflow raise never made the flyout visible so every overflow ref failed the actionability occlusion check, and a taskbar-rooted tray click refused on agent visibility. The tray click capability is written into §2.15's scope (dogfood 2026-08-28, A26-5, A26-7). `focused_window`'s frame-vs-`CoreWindow` identity is measured against a real `ApplicationFrameHost`-hosted target, with no `PLATFORM_NOT_SUPPORTED` hedge: satisfied — foreground reports the `ApplicationFrameWindow` owned by `ApplicationFrameHost` with the `CoreWindow` child owned by the hosted application (A26-8). The Chromium/Electron menu-detector arm is evaluated: satisfied — a real staged Chromium DOM menu left both shipped sources silent, and the framework-gated third source added for it fires under the staged menu and stays false at rest (A26-12). The WinUI arm of §2.11's menu detector: the at-rest half is evaluated against the activation-confirmed UWP host and no generic staging raised a reachable menu surface, so the unevaluated menu-open arm is written into §2.15's scope with the staging attempted and the population searched (A26-11) — carried, not closed. A resolve-identity fallback for zero-identity Chromium content leaves ships: satisfied — allocation strips a ref's bounds rect only when the entry carries some other identity to resolve by (A24-11); the leaf-population classification stayed unmeasurable this session and rides §2.15 beside the detector verification (A26-13). Windows `launch` stays path-or-system-image, by recorded decision rather than by omission (A21-8).
...
|---------|---------|-------------------|
| `open-system-surface --surface <kind>` | Opens an OS shell surface so agents can immediately call `snapshot --surface <kind>` and act by refs | Windows kinds: `start-menu`, `taskbar`, `system-tray`, `system-tray-overflow`, `action-center`, `quick-settings` — build-dependent. On this build the Start-menu accelerator raises a search-hosted `CoreWindow` rooted at `PaneRoot`/`SearchTextBox` rather than a tile surface (A26-9), and the quick actions are a pane inside the Action Center itself, so `quick-settings` resolves `PLATFORM_NOT_SUPPORTED` with `platform_detail` naming `action-center` (A26-3, C-10). macOS may support `spotlight`, `dock`, `menu-bar-extras`, `notification-center` — the macOS kinds keep the trait default through Phase 2 and ride §2.15. Unsupported kinds return `PLATFORM_NOT_SUPPORTED` |
| `list-tray-items` / `click-tray-item` / `open-tray-menu` | Not shipped: measurement moved the notification area under this table's own generic rule. Its items are `Button` elements with stable machine-local `AutomationId`s and `InvokePattern` on positive-area bounds, so they carry refs and the stated preference applies — `snapshot --surface system-tray-overflow` (five items) exposes them, and the promoted toolbar's three items surface through `snapshot --surface taskbar` (A26-5, A26-7); the dedicated `system-tray` surface's promoted read returned zero items at the dogfood and no tray click delivered live, so the click path rides §2.15 | Reached through `Shell_TrayWnd` plus the overflow flyout (`TopLevelWindowForOverflowXamlIsland` on Win11 22H2+; `NotifyIconOverflowWindow` before that — present and readable first-party on this build, C-5, A26-6); macOS maps to menu bar extras. Linux maps to StatusNotifier in Phase 3 |
...
- **Primary list path:** open Action Center with `open-system-surface --surface action-center` and traverse its UIA tree: a `ListView` at `MainListView` under the Action Center `CoreWindow`, a top-level `ClearAllButton`, and per-entry `Title`/`Attribution` `Text` children with a `DismissButton` carrying `Invoke`, all addressed by `AutomationId`; richer per-entry affordances (`Content`, `ExpandButton`, `VerbButton`) are presence-dependent on the notification's own content and are checked per entry rather than assumed from the tree shape (A26-3).
- **The `UserNotificationListener` path is not consulted.** On the measured host its WinRT activation succeeds unpackaged and `GetAccessStatus()` reads `Denied` with `RequestAccessAsync` never called; the `userNotificationListener` consent store reads `Deny` and the `NonPackaged` key is absent. Two explanations fit those readings — no package identity to grant the capability against, or a machine-wide privacy consent a user could flip — and no cause is claimed: the readings are recorded (A26-4) so a future host can re-decide on evidence, the listener's runtime `PERM_DENIED` branch is not reached on the shipped path, and a shipped binary cannot depend on per-machine consent state.
- **Dismiss:** invoke the entry's `DismissButton` through the Action Center tree; the listener is not consulted (see the list paths above). For "dismiss all", invoke the shell's `ClearAllButton` only when present.
...
- **Edge case:** some notifications are transient (disappear after timeout). On Windows `wait --notification` polls the Action Center listing path within the normal wait deadline — the listener is not consulted (A26-4), so no event subscription backs the wait and the poll is the path.
...
- **Open menu:** a right-click at the tray item's bounds raises its context menu, and the popup is addressed by the `menu` surface for ref-based interaction. The advertised surface set gains `Menu` together with the `surface_root` arm that resolves it, so an open tray popup snapshots like any other surface; the right-click is the raise itself (A26-7).
...
- **Extend `trace_sanitize`'s field list to the full notification envelope.** `SENSITIVE_KEYS` (`crates/core/src/trace_sanitize.rs`) covers `title` and `app_name` (via token matching, since `trace_key_tokens` splits keys on case and non-alphanumeric boundaries, so `app_name` yields the token `name`), but not `body` and `actions` — the two remaining serialized `NotificationInfo` field names — so any future trace site emitting an entry payload would reach on-disk trace segments and the FFI log callback unsanitized. Measured at the dogfood: no adapter's notification path emits payload-bearing trace events today (a traced Windows `list-notifications` and `dismiss-notification` carried only meta/start/end segments, with the staged content's literals absent from the trace file), so this is a gap in the safety net rather than a live leak — the cheapest time to close it. What closes it: the two field names (`body` and `actions`) added to `SENSITIVE_KEYS`, with regression-pin tests for `app_name` and `title` (already covered by token matching) and invert-verified tests for the new fields, one core edit shared by both adapters and the FFI log layer. **Evidence:** dogfood 2026-08-28 findings; §2.14's plan risk on `trace_sanitize`
- **Re-baseline the two live E2E legs that fail identically at this branch's merge-base.** `headed-double-click` (actionability preflight `hit_test` occluded 5/5 on an unattended desktop) and `interaction-scroll-to-visibility` (scroll-to `ACTION_FAILED`) fail the same way when this sub-phase's diff is stashed, so neither is introduced here; the dogfood independently observed the occlusion half live — a tray-click preflight's 5/5 occluded hit-tests named the driving console's own window as the occluder. What closes it: a harness stage whose occluder is the harness's own topmost fixture rather than the operator's console, on the self-hosted runner this gate registers. **Evidence:** merge-base stash run; dogfood 2026-08-28 leg 2
- **Clean up the `agent-desktop-windows` crate's Linux cross-compile, which does not compile for `x86_64-unknown-linux-gnu`.** The breakage is pre-existing and outside the shell-surfaces sub-phase's files — re-measured 25 errors at this branch's HEAD, all unresolved items behind `#[cfg(target_os = "windows")]` — and grows from the merge-base where the branch's own new stub modules carry non-Windows entry points but leave stub cross-references unresolved. No CI lane compiles the platform crate for a non-Windows target, so a crate that cannot build for Linux merges green — the exact shape the repo learning names: platform code no lane executes or compiles is a hypothesis, not shipped code. What closes it: a `cargo check -p agent-desktop-windows --all-targets --target x86_64-unknown-linux-gnu` lane (or a recorded exclusion with its reason) plus the stub repairs the check surfaces. **Evidence:** merge-base stash run; `docs/solutions/best-practices/never-ship-platform-code-that-ci-cannot-execute.md`
- **Take the A26-13 nameless-leaf population classification on a session where a Chromium tree actually settles.** Two attempts have now failed to produce the settled tree the measurement needs: A26-13's own (the Chromium UIA tree pinned at or below a 34-node exposure floor through 60 s of settling) and §2.14's dogfood re-attempt (Obsidian had self-updated 1.12.7 → 1.13.7 since the ledger; the product's fresh-client snapshots pinned at a 15-ref skeleton through 60 s of settling with the window front and focused, and the vault picker's DOM controls never exposed) — so the positive-area versus zero-extent classification of nameless content leaves still has no qualifying population, and the zero-identity resolution fix (A24-11's half) is live-verified while its rectangle premise is still uncounted. What closes it: one session where a Chromium content tree exceeds the exposure floor (A24-11's 262-ref content precondition remains the shape to reproduce), then the two-count classification. **Evidence:** A26-13, A24-11, A1-5, A1-6
- **The three ledger rows this gate owns are closed here, by measurement.** `A27-2` recorded a child enumeration that could not see a window the product's identical call did see, and deliberately handed the cause to a reproduction rather than asserting one. `A28-4` and `A28-5` are that reproduction: three `ToolbarWindow32` windows sit under `Shell_TrayWnd`, two of them collapsed zero-extent placeholders whose automation elements genuinely have no children, and the shipped class chain stopped one hop short and resolved one of the placeholders. All three rows are **closed** by adding the missing `SysPager` hop, after which `--surface system-tray` returns three refs and a click on one is delivered. The descend-the-taskbar-tree approach an earlier attempt built is **disposed of** as unnecessary and removed, and the claim that it improved the overflow surface is corrected in the rows themselves - overflow already read five refs without it.

- **Retire the live e2e CI lane rather than register a runner.** §2.12 shipped the registration artifacts - `windows-e2e.yml`, the fork-PR approval policy, the ephemeral-versus-persistent decision, `scripts/run-windows-e2e-ci.ps1` with its refusal guard, and `docs/runbooks/windows-self-hosted-runner.md` - with no runner to execute them, because the owner declined registration on a public repository. This gate asked again and the owner declined again, and directed that the lane be removed from CI. The reason is unchanged and is not a scheduling one: a self-hosted runner's labels are reachable from every `pull_request`-triggered workflow in the repository, not from the single file that names them, so a fork PR editing any of those workflows is code execution on the owner's interactive desktop - the exposure GitHub's own public-repository guidance warns about. What ships: `windows-e2e.yml` is deleted, along with `ci.yml`'s static assertion that policed its capture upload (a check whose whole purpose was to gate a file nobody could execute); the queued `push` run that had accumulated against it is cancelled; `docs/runbooks/windows-self-hosted-runner.md` is kept unexecuted and rewritten to describe a retired lane, so a future registration has a reviewable policy rather than one invented under pressure. **The live Windows e2e suite still runs - locally, on demand**, with `scripts/run-windows-e2e-ci.ps1` under the exclusive `DesktopLease` on a box holding the interactive session, which is how every scenario in it has been verified since §2.12. CI keeps what does not need a desktop and already covers the Windows surface: `cargo test -p agent-desktop-core -p agent-desktop-windows --lib` on GitHub-hosted `windows-latest` **and** `windows-11-arm`, the x64/ARM64 lib-test parity job, clippy over the Windows crates, the Windows example tests, the e2e contract gate and its self-test half, the seeded-failure run, the refusal-guard self-test, the capture-redaction gate and the phases.md citation gate. **R20, R21 and R22 are dispositioned, not met**, and the disposition is retirement: there is no receiving sub-phase and no infrastructure waiting to be provisioned
- Live e2e in both headless and headed modes on the Windows runner registered above — this is the half of §2.12's live-gate exit criterion that could not be discharged there
- **Multi-monitor `list_displays` verification and per-display capture.** §2.12 measured this box at a single display (grounding G2, virtual screen `0,0 1639x732`) and a software/virtual second display unmanufacturable on this class of box (`A22-8`, `manufacturable: false`). §2.12 registered no runner at all, so this is a provision-or-ratify decision at the one gate that reviews the whole branch: either provision a rig presenting more than one monitor, ideally at mismatched DPI, and close `list_displays`'s aware-versus-unaware bounds delta (A10-3, A16-4), A18-9's mixed-DPI verification, and per-display `screenshot --screen <index>` correctness together, or ratify that the divergence cannot be closed within this phase and record why
- **Live elevation-required launch effect (`ERROR_ELEVATION_REQUIRED`, 740).** A21-2 pinned the `740` → `HRESULT_FROM_WIN32` mapping but could not stage the live effect: a Medium-integrity caller launching a `requireAdministrator` target needs a nested Medium→elevated-launch boundary that no probe host, including §2.12's `CreateRestrictedToken`-staged rig, manufactures — `CreateRestrictedToken` lowers the caller's own integrity, it does not stage a UAC elevation prompt for a child process. This rides the same provision-or-ratify decision as the two rig items above, on the same ground (*dependent on infrastructure that does not exist*): a host where a Medium-integrity caller can genuinely attempt a `requireAdministrator` launch and observe the result closes it; ratifying that Windows ships the pre-committed `740` mapping with no live-effect proof is the alternative
- **Capture session-degradation under RDP, a locked desktop, or Session 0 — and, folded into the same rig decision, 2.0's deferred RDP/session-isolation row (`A10-2`).** Producing the condition needs locking or disconnecting an interactive session, and §2.12's one available session cannot be unlocked from inside itself — a failed reattach strands the box — so this needs a second, disposable interactive host that can be disconnected without loss. What it must measure: whether UIA observation, capture and input behave identically on a disconnected or locked session and on Session 0, and whether the console-session-agnostic claim this document currently makes still holds once a non-console session actually exists to test it against
- **Live modern-capture pixel success, if §2.12's own hosted capability-probe reading did not already prove it.** §2.12's Deferred to Follow-Up Work item 5 pre-commits both branches: proven hosted, the obligation is already discharged there and this bullet is inert; not proven, `A22-1` measured `IsSupported: true` with interop failing on a build-17763 host, so a host whose WGC interop actually works is what closes it, and it is provisioned or ratified alongside the two rig items above
- **Implement macOS's `open-system-surface` kinds — `spotlight`, `dock`, `menu-bar-extras` — or ratify that macOS stays without them.** P2-O14's macOS half kept the trait default through the Windows run: §2.14 shipped only the Windows shell surfaces, so the objective reads closed only if this gate either implements the three macOS kinds through the same resolve-then-snapshot seam §2.14 exercised (`resolve_shell_surface`'s adapter method is platform-neutral and macOS has never implemented it) or records the ratified divergence with its reason. The portable `NotificationCenter` surface already ships on macOS; these three shell kinds are the open remainder of P2-O14
...
- **Settle the `offscreen` cross-platform contract.** The token means different things on the two adapters and this is deliberate for Phase 2, not an oversight: macOS computes it geometrically from element-versus-window bounds (`crates/macos/src/tree/state_reader.rs:86-95`), while Windows emits UIA's provider-reported per-element value (sub-phase 2.3). They genuinely disagree — a virtualized row scrolled out of view is geometrically inside the window but offscreen to the provider — and A14-8 measured a UIA value contradicting itself *within one window*, where a minimized top level reported `IsOffscreen` true while every descendant reported false, which is why no adapter may propagate a container's value to its subtree. Decide here whether the contract specifies one meaning, and if so which, now that both adapters exist and there is evidence from both. Identical JSON across platforms is a product promise, so a permanent divergence is a decision to take explicitly rather than inherit. This gate owns the decision and the record of it, not its implementation: if the decision is to normalize, the normalization changes shipped behaviour on at least one adapter, and it lands in this gate, verified against both adapters, in a commit scoped to that one normalization so it reads on its own. Ratifying the divergence needs no successor PR
- **Settle the `button`/`switch` role and `pressed`/`checked` state divergence for the same toggle control.** This is deliberate for Phase 2, not an oversight: macOS keeps the control's role as `button` and reads its toggle value as `pressed` (`crates/macos/src/tree/state_reader.rs:57-59`), while Windows reclassifies any `Button` control type that advertises `ToggleAvailable` to `Role::Switch` before states resolve (`crates/windows/src/tree/roles.rs`'s `button_role`), so the same control surfaces as `switch` with state `checked` instead and `pressed` stays permanently unproduced on Windows — `crates/windows/src/tree/states.rs`'s `resolve_states` doc comment explains why that precondition can never hold there. Decide here whether the contract normalizes the role, the state token, or both, or ratifies the divergence explicitly, using the same identical-JSON-is-a-product-promise standard the `offscreen` decision above uses — and on the same terms: a normalization is a shipped-behaviour change that lands in this gate, in a commit scoped to that one normalization so it reads on its own, while a ratified divergence is recorded here and needs no successor
- **Settle the UWP-hosted window identity split — a window `id` owned by one process and an `app`/`pid` naming another.** On an `ApplicationFrameHost`-hosted target the window `id` is a handle owned by `ApplicationFrameHost` while `app` and `pid` name the hosted application: the frame carries the title, position and foreground, and the hosted application's identity is read one level down from its `Windows.UI.Core.CoreWindow` child — measured end to end with Settings frontmost (A26-8), matching A1-3's static split. §2.14 ships the descent so a hosted application reports one identity to every command that names it; what this gate decides, beside `offscreen` and the role/state decisions above, is whether the cross-platform contract documents the handle/pid split or normalizes it — a caller reading either field alone cannot see it, and the identical-JSON-across-platforms standard the other divergence decisions here apply is the bar. This is the identity residual §2.12 recorded, not the pid mismatch §2.14's `--app` descent fixes
- **Evaluate the WinUI3/MSIX arm of the menu detector against a real WinUI3/MSIX host.** The UWP half of the arm went as far as this build allows and no further: with the Settings host activation-confirmed through the `ms-settings:` route, both shipped detector sources were read directly at rest (false), and three generic stagings — an Alt tap, Shift+F10, and a right-click at the `CoreWindow` center — staged no reachable menu surface, so the menu-open half is unevaluated with `measurable: false` narrowed to WinUI3/MSIX (A26-11). The population searched: no WinUI3/MSIX host exists on any box measured to date (A10-7); the UWP host was reached and evaluated, and an external process cannot summon another application's XAML `MenuFlyout`. What closes it: a host carrying a WinUI3/MSIX application whose menu surface can be staged externally, evaluated through the shipped detector sources
...
- **Settle the `type` command's cross-platform contract, which has no semantic-headless path on Windows.** macOS headless `type` inserts at the selection by writing `AXSelectedText` (`crates/macos/src/actions/type_text.rs`), a semantic write needing no focus steal, so strict-headless `type` succeeds there. Windows UIA exposes no insert-at-selection equivalent: `ValuePattern.SetValue` replaces the whole value (that is `set-value`, §2.7's headless text path) and `TextPattern` is read-only for insertion, so §2.8's `type` is physical synthesis under the focus-fallback/headed policy and strict-headless `type` fails at policy where macOS would have typed. Decide here — with both adapters in review — whether the contract normalizes (for example by defining `type` as semantic-where-available and documenting the per-platform policy floor), or ratifies the divergence explicitly, on the same identical-JSON-is-a-product-promise standard the `offscreen` and role/state decisions above use and the same terms: a normalization is a shipped-behaviour change landing in this gate, in a commit scoped to that one normalization so it reads on its own, while a ratified divergence is recorded here and needs no successor. **Evidence:** §2.8 scope (`type` physical-only), A4-1 (`KEYEVENTF_UNICODE` synthesis measured working), §2.7's `set-value` headless path
- **Settle the two `press --app` divergences Windows cannot match.** Both stem from Windows `SendInput` injecting into the foreground queue with no per-pid targeting, where macOS delivers to a specific pid. **(a) No semantic accelerator path:** macOS's `press_key_for_app` (`crates/macos/src/system/key_dispatch.rs`) first walks `AXMenuBar` for a menu item whose `AXMenuItemCmdChar`/`AXMenuItemCmdModifiers` match the combo and presses it via `AXPress` — a semantic delivery needing no focus steal — while Windows exposes no queryable global menu-bar/accelerator surface, so §2.9's `press_key_for_app` is synthesis-only. **(b) No headless delivery to a background target:** macOS's headless arm (`allow_focus_steal` false) delivers via a pid-targeted event regardless of foreground, while Windows cannot inject to a non-foreground window without stealing focus, so a headless `press --app` whose target is not already frontmost fails closed on Windows where macOS succeeds. Decide here — with both adapters in review — whether the contract normalizes (for example by defining `press --app` as semantic-where-available with a documented per-platform focus floor) or ratifies the divergence explicitly, on the same identical-JSON-is-a-product-promise standard the `type` decision above uses and the same terms: a normalization lands in this gate, in a commit scoped to that one normalization so it reads on its own, a ratified divergence is recorded here and needs no successor. **A third arm surfaced at §2.11's dogfood: the non-interactive *caller*, distinct from the non-foreground *target*.** `press escape --app <image>` issued from a background job failed to dismiss an open native menu in 4 of 4 reproductions while the identical command from the interactive foreground console dismissed it every time. The envelope is honest — `SendInput` cannot verify delivery, so `delivered_unverified` is the correct disposition rather than a false success — but the reach limit is undocumented, and an agent driving Windows from a service, a scheduled task or a CI job is exactly the caller that hits it. Settle it alongside the two arms above. **Evidence:** §2.9 scope (`press_key_for_app` synthesis-only, headless-non-foreground fails closed), macOS `key_dispatch.rs`'s `try_menu_bar_shortcut` and its pid-targeted `synthesize_key`, §2.8 KTD2 (`SendInput` foreground-queue, no per-pid targeting), §2.11 dogfood F3
- **Settle the Windows `launch` identifier contract (system-dirs-only bare names).** §2.9's `launch_app` accepts an absolute path or a bare name resolvable only under `System32` / the Windows directory (A21-1); it deliberately does not follow `CreateProcessW`'s full module search order and does not read `App Paths` / Start Menu. macOS resolves installed apps by display name or bundle id through `NSWorkspace`. §2.14 recorded the decision not to take the by-name/AUMID path — the `Win32_UI_Shell` + `Win32_System_Registry` gate stands, and a hand-declared `IApplicationActivationManager` reaches only packaged AUMIDs (A21-8 validated the binding in standalone scratch with `expand_2_9: false`) — so the Windows side arrives here settled: Windows identifiers are path-or-system-image. Decide here only whether the portable `launch` contract normalizes or ratifies that Windows identifiers are path-or-system-image while macOS identifiers are display-name-or-bundle-id. **Evidence:** A21-1, A21-8, §2.9/`launch_path.rs`, §2.14's recorded decision
- **Settle what `--app` scoping means for an event wait, because three of its consequences surprise a caller and one contradicts the flag's own description.** `signal_filter` resolves `--app` through `resolve_app` before the poll loop for every event except `app-launched` (`crates/core/src/commands/wait_event.rs`), and pins the wait to the single process instance that resolution returned. Measured at §2.11's dogfood: scoping to an application that is not running yet returns `APP_NOT_FOUND` in under 100ms while ignoring the timeout entirely, so an agent racing a concurrent launch gets a hard failure instead of the budget it asked for; a target that dies *during* resolution returns the same code, which reads as a bad `--app` value when the truth is the opposite - the application existed and its disappearance is what broke the lookup; and a second process of the same image name starting mid-wait is invisible for the rest of that wait, a silent miss the unscoped path catches. The flag is documented as "scope the wait to a specific application", which a reasonable caller reads as processes named X rather than one instance of X. §2.11 corrected the shipped documentation to describe the real semantics; what is undecided is whether the *behaviour* should change - whether a scoped wait re-resolves, whether an appearance-class event should defer resolution into the loop, and whether a disappearance-class event should treat an unresolvable target as its answer rather than an error. Platform-neutral and shared with macOS. **Evidence:** §2.11 dogfood F8, F11, F12
- **Settle batch's event-wait pre-seed, which is one entry too early.** `src/batch/execution.rs` captures the `SignalBaseline` for an event wait *before dispatching the preceding entry*, which is a genuine improvement for `[launch, wait window-opened]` - the pre-launch baseline makes the new window's appearance a real diff - and defeats `[launch, wait window-closed]` completely: the seed is taken before the application exists, so the window is absent from the baseline and absent from the final state, and the diff sees no net change at all. The wait runs its full timeout and reports `baseline_counts.apps: 0` immediately after a `launch` succeeded in the same batch. This lives in the platform-agnostic binary crate and is reachable identically on macOS. Decide whether the pre-seed moves to after the preceding entry for disappearance-class events, or whether the batch contract documents the asymmetry. **Evidence:** §2.11 dogfood F10
- **Settle whether `wait --event`'s baseline advances, because today a disappearance is invisible for anything that appeared after the wait started.** `wait_for_event` seeds a baseline from the first successful capture and diffs every later poll against that same fixed capture, never advancing it (`crates/core/src/commands/wait_event.rs`). An entity that both appears and disappears inside one wait is therefore absent from the baseline and absent from the current capture, so `diff_signals` sees nothing. Measured on Windows at §2.11's dogfood: a window opened and closed inside one wait produced no `window-closed` in 4 of 4 discriminating trials while a pre-existing window produced one in 4 of 4, and an application launched 2s into a 14s wait and terminated 3s later produced `TIMEOUT` rather than `app_terminated`. The gap generalises to every disappearance event — `window-closed`, `app-terminated`, `surface-dismissed`. **This is core, not adapter:** the fixed baseline is platform-neutral and macOS behaves identically, so it is not a Windows defect and could not be fixed from a Windows sub-phase. Decide here whether the contract keeps the fixed baseline (a wait answers "what changed since I started"), advances it per poll (catching transient lifecycles at the cost of redefining every event), or documents the limit explicitly — on the same identical-JSON-is-a-product-promise standard the entries above use, and the same terms: a normalization is a shipped-behaviour change landing in this gate, in a commit scoped to that one normalization so it reads on its own. **Evidence:** §2.11 dogfood F1, `docs/dogfood-reports/2026-08-15-001-feat-windows-2-11-signals-wait-parity-dogfood.md`
- **Settle `wait --notification`'s per-poll open/close cost — the conditional follow-up §2.14's wait decision pre-committed has fired.** The wait opens and closes the Action Center per poll (each poll runs in its own one-call session, exactly as on macOS), and that shape was shipped over a rejected held-open watch session accepted on the condition that a measured per-poll cost would force the question. The condition fired: §2.14's product cost baseline measured `list-notifications --headed` at **min 1243.5 ms (median 1254.2 ms) per invocation** — raise + read + close per call, taken through the release binary against an empty center — so every wait poll pays the full shell-raise cycle. Decide here, with both adapters in review: keep per-poll open/close as the accepted platform cost, or introduce a held-open watch session for the wait's duration. A held-open session changes shipped behaviour, so a normalization lands in this gate, in a commit scoped to that one normalization so it reads on its own; ratifying per-poll open/close is recorded here and needs no successor. **Evidence:** dogfood 2026-08-28 cost baseline (`docs/dogfood-reports/2026-08-28-001-feat-windows-2-14-shell-surfaces-notifications-dogfood-captures/cost-baseline.json`)
- **Settle whether Windows `--app` accepts an application's stem as well as its image name.** Windows resolves `--app` through `list_apps_scoped`, whose core default matches `AppInfo.name` with `eq_ignore_ascii_case`, and `AppInfo.name` on Windows is the raw image name from the process table. So `--app notepad.exe` resolves and `--app Notepad` returns `APP_NOT_FOUND` — while macOS accepts a display name or bundle id, and the substring predicate `WindowFilter.app` uses *would* have matched. §2.11 traced the three sites core matches at and adopted the image name rather than adding a fourth semantics, explicitly rejecting a Windows override of `list_apps_scoped` that would accept the stem: it would make `--app Notepad` work and is genuinely more usable, but it changes app resolution for **every** Windows command — `close-app`, `focus`, `snapshot --app`, `press --app` — which is §2.9's contract and a wider blast radius than a wait sub-phase may take unreviewed. §2.11's dogfood then confirmed the failure from the outside, and it is the shape the entry below anticipates: a caller who passes a display name gets a bare `APP_NOT_FOUND` with no hint toward the accepted form. §2.14's dogfood measured the divergence **between two Windows commands** on the same host: `list-windows --app <stem>` succeeds (zero rows, the window-filter predicate) while `list-surfaces --app <stem>` refuses `APP_NOT_FOUND` and the image name resolves both, so the accepted identifier form differs per command family, not only per platform. Decide here, with both adapters in review, whether Windows accepts the stem, whether the identifier contract is documented as platform-specific, or whether the divergence is ratified — on the same identical-JSON-is-a-product-promise standard the entries above use, and the same terms: a normalization is a shipped-behaviour change landing in this gate, in a commit scoped to that one normalization so it reads on its own. **Evidence:** §2.11 KTD5's three-site trace, §2.11 dogfood F4, §2.14 dogfood leg 4, and `AppInfo::matches_identifier` (added in v0.8.1 and documented as "the one predicate every launch-target match uses") reaching the launch path, `list_apps_scoped` (core's default at `crates/core/src/adapter/observation.rs:61`, which Windows does not override), and `process_from_baseline` (`crates/core/src/commands/wait_event.rs`)
...
- **Settle whether the mutation-path delivery classifier promotes into `agent-desktop-core` beside the resolver-payload item.** §2.7 ships Windows `actions/mutation.rs` mirroring macOS `ax_mutation::classify`: both adapters pin the same `ErrorCode` / `DeliveryDisposition` / `RetryDisposition` pairings, but each classifies a platform-native failure space (`AXError` vs HRESULT/`UiaFailure` sentinel) and the outcome contract is deliberately per-adapter for Phase 2. Decide here — with both adapters in review — whether a shared mutation-outcome type or pairing table belongs in core, or whether the mirrored pairings stay adapter-local with the wire shapes as the only shared contract. A promotion that changes macOS to consume a core type lands in this gate, in a commit scoped to that one normalization so it reads on its own; ratifying the per-adapter split is recorded here and needs no successor. **Evidence:** §2.7 scope (mutation classifier), A19-2 failure taxonomy, 2.7 dogfood envelope parity
- **Answer a ref action terminally when its owning process is gone, instead of retrying to the deadline and blaming the wrong cause.** A ref taken from a live application, whose process is then killed, is still retried by `resolve_element_strict`'s `retry_incomplete_until(deadline, ...)` loop (`crates/windows/src/tree/resolve.rs`) until the budget is exhausted, and the caller receives `TIMEOUT` with `suggestion: "The target application may be busy or unresponsive"` and `recovery: null`. Measured at §2.12's dogfood: 5,091 ms at the default budget and 1,127 ms at `--timeout-ms 1000` - the full budget in both cases - while `snapshot --app` against the same dead process returns `APP_NOT_FOUND` immediately, so the product already detects the condition on a neighbouring path. The envelope is misleading rather than merely imprecise: `TIMEOUT` with `retry: safe` invites a retry against a process that cannot return, and `recovery` is empty where `STALE_REF`'s `refresh_snapshot_then_retry_original` would have named the one action that makes progress. **What closes it:** a vanished owning process is classified terminally on the first attempt rather than as an incomplete read, so the ref action returns `STALE_REF` / `not_delivered` with the refresh strategy attached. **Why here:** the change is to the resolver's retryability classification, which is §2.5's contract and is reached by every ref action on the adapter - click, set-value, toggle, expand, select, scroll - and it sits in the same code path as the resolver-payload promotion and the `stale_ref` constructor entries above, so the three are one edit under one review. **Evidence:** `docs/dogfood-reports/2026-08-16-001-feat-windows-2-12-fixture-e2e-harness-dogfood.md` F1
- **Decide `AppInfo.presentation` alongside `renderer`.** Windows populates `name`, `pid` and `process_instance` on every `list-apps` row and leaves `presentation` absent, while macOS sets it. Identical JSON across platforms is a product promise, and an agent written against macOS that branches on the field silently takes the absent path on Windows rather than failing loudly. Both fields are `LaunchResult`/`AppInfo` metadata macOS derives from the application bundle and Windows has no equivalent source for, so deciding one without the other splits a single contract question across two gates. **Evidence:** §2.12 dogfood F3
- **Stop handing `AdapterError::stale_ref` a sentence where it expects a ref id.** `stale_ref` (`crates/core/src/adapter_error.rs:73`) takes a **ref id** and formats `"{ref_id} not found in current RefMap"`. Fifteen call sites hand it a whole sentence instead, so the message an agent reads and acts on comes out as `"Saved target has no process instance identity not found in current RefMap"` — ungrammatical, and wrong about the cause in every one of them: the ref was found and read, and it was the live evidence, the process generation, or the input geometry that refused it, never a missing RefMap entry. Ten sites are on macOS — `tree/resolve_errors.rs:4`, `tree/resolve.rs:52`/`:59`/`:62`, `tree/query/mod.rs:144`/`:149`/`:152`, `tree/renderer_probe.rs:9`, `actions/post_state.rs:115`, `actions/physical_click.rs:75` — **four are in core itself**, so they are already reached on every platform: `ref_action.rs:65`, `headed_focus.rs:53`, `renderer_accessibility.rs:58`, `snapshot_ref.rs:145` — and **one on Windows:** `crates/windows/src/actions/physical_target.rs:91`. **What closes it:** each of the fifteen builds its error directly — `AdapterError::new(ErrorCode::StaleRef, message)` with the snapshot-refresh suggestion and the not-delivered disposition — exactly as Windows's `stale_evidence_error` (`crates/windows/src/tree/resolve_match.rs`) already does, leaving `stale_ref` to the three callers that genuinely pass a ref id (`crates/core/src/refs_store.rs:76`, `crates/core/src/commands/helpers.rs:317`, `crates/core/src/snapshot_ref.rs:59`) and a test that pins the constructor's message against a caller passing anything but an id. **Why here:** ten of the fifteen are in the macOS crate, the GA line for the whole platform phase, and the error-payload promotion above already opens `resolve_errors.rs` and `resolve.rs` — the same family of defect in the same two files, so this is one edit under one review rather than two. The four core sites and one Windows site travel with them rather than shipping a message set that reads one way in core and another in the adapters. **Evidence:** found while sub-phase 2.5 built the Windows resolver against the same constructor and rejected it for exactly this reason (`resolve_match.rs`'s recorded rationale)
- **Fix or ratify Medium→High `focus-window` not failing closed.** §2.9 shipped an integrity comparison meant to fail a strictly-higher-integrity `focus-window` closed as `PERM_DENIED`/`not_delivered`; §2.12's split-integrity harness (`tests/e2e-windows/scenarios/SplitIntegrity.ps1`, `Invoke-SplitIntegrityActivationLeg`) measured this live and found the opposite: staged against a genuine `CreateRestrictedToken`-confirmed Medium caller, with a foreground precondition ruling out a trivial no-op success, `focus-window` against the High-integrity fixture returns `ok:true` and a raw `GetForegroundWindow` re-read confirms the target genuinely took foreground. `SetForegroundWindow` is gated by Windows' foreground-lock-timeout heuristic (recent input, `AllowSetForegroundWindow`), not by Mandatory Integrity Control — UIPI covers `SendInput`/`PostMessage`, which §2.12 separately confirmed *does* fail closed for a Medium→High `set-value`/`type`, but not window activation. Decide and ship one of: an explicit pre-check in `focus_window` that compares integrity and refuses a strictly-higher target before calling `SetForegroundWindow` at all (matching what §2.9's own documentation already claims the behavior to be), or a corrected, ratified contract stating that window activation crosses the integrity boundary while input delivery does not. Either way, `docs/phases.md`'s §2.9 scope, the class-(b) envelope list above, and §2.12's own scope must read true against whatever ships. **Evidence:** `tests/e2e-windows/scenarios/SplitIntegrity.ps1`, live run against `AgentDeskFixture.exe`, integrity confirmed by token read-back (`S-1-16-8192`)
- Hand §2.16 a settled contract and a proven branch. **This gate does not merge to `main`** — §2.16 is the last sub-phase and carries the promotion, which its own Sequencing and Exit criteria state

**Settled contract (decided here, so the next platform inherits answers rather than questions).**

- **`offscreen` means one thing and is measured at two fidelities, and that is ratified rather than normalized.** The token means *no part of the element is presently rendered within the visible region of its container*. macOS approximates that geometrically against the window rectangle, because it has no other source — no macOS element publishes `AXOffscreen`. Windows reads the provider's per-element value, which additionally sees scroll-viewport clipping that no window-disjointness test can reproduce (A18-1 measured a list item flipping the value over a 40px scroll). Normalizing was attempted twice on paper and rejected on measurement: Windows has no window rectangle to compute against — `resolve_states` receives no rectangles and the crate has no `window_bounds` concept — and `IsOffscreen` additionally feeds the actionability visibility gate, `scroll_ladder` and `scroll_into_view`, so dropping it would stop a below-fold target failing that gate. **A14-8's ban binds both adapters: a container's value is never propagated to its subtree.**
- **The role does not diverge; only the state token did, and Windows now produces it.** `crates/macos/src/tree/roles.rs` maps `AXSwitch`/`AXToggle` to `switch`, so a genuine switch is `switch` on both adapters and there was never a role divergence to settle. What Windows lacked was `pressed`, and it had the information all along: `LegacyIAccessibleState` is already in the walk set and already read. A `button` carrying `STATE_SYSTEM_PRESSED` now emits `pressed`. `button_role`'s reclassification of toggle-capable buttons to `switch` is untouched and correct — measurement found it accurate on every control that reached it, and the toolbar toggle button that was missing `pressed` advertises no `TogglePattern` at all.
- **Dangerous-shortcut matching answers by one rule on both adapters: the pressed modifiers must be a superset of a listed entry's.** An enumeration cannot cover a variant nobody listed. macOS's `BLOCKED` list is deliberately **not** edited — two entries become redundant under the rule and keeping them makes the change monotonic, so nothing blocked before can become unblocked regardless of whether the redundancy argument holds.
- **`type`, `press --app` and the `launch` identifier are ratified divergences, because Windows cannot reach macOS's behaviour rather than choosing not to.** UIA exposes no insert-at-selection, so strict-headless `type` returns `POLICY_DENIED` where macOS writes `AXSelectedText`. `SendInput` has no per-pid targeting, so Windows has no semantic accelerator path and no headless delivery to a background target — **and a third arm is documented with them: a non-interactive *caller* (a service, scheduled task or CI job) reaches a foreground-gated target no more reliably than a background target does.** Windows `launch` identifiers stay path-or-system-image against macOS's display-name-or-bundle-id. Each is documented per-platform with its floor rather than papered over.
- **The UWP-hosted window identity split is documented, not normalized.** The frame's handle stays the window `id` because the frame is what the desktop treats as a window; `app` and `pid` are read from the `CoreWindow` child. Normalizing would hand out an identity `focus-window`, `move-window` and `resize-window` would then fail against.
- **The mutation-path delivery classifier stays adapter-local.** The pairing sets are **not** identical — Windows carries `ActionFailed`/`NotDelivered` for `UIA_E_ELEMENTNOTENABLED` and `Timeout`/`DeliveryUncertain` for `UIA_E_TIMEOUT`, neither of which macOS has a branch for — so a shared core type would have to model two genuinely different failure spaces. A conformance test asserting both classifiers land only in core's shared vocabulary is the guard, without the abstraction.
- **An application is a window-owning process on Windows, and that is ratified** for `close-app` and for the lifecycle-event population alike. The one wrong answer it produced is fixed rather than ratified: `app-terminated` is now confirmed against process liveness before it is reported, failing open so an unreadable process state still emits the event.
- **Launcher-style child-pid attach keeps the pid-exact rule**, and `AMBIGUOUS_TARGET` stays the answer for a multi-row image match. One thing is fixed rather than ratified: supplying arguments suppresses the attach path, because passing arguments is a distinct intent from attaching to what is already running.
- **`presentation` stays macOS-only; `renderer` does not.** `presentation` derives from an activation policy Windows has no equivalent for at any point. `renderer` is different: `observe_window` resolves the window *before* `launch_result` is built, so a launch that produces a window can classify it with the shipped window-class detector. Windows sets `renderer` whenever the launch resolved a window; a launch that resolved none reports nothing, because there is nothing yet to classify.
- **Window activation crosses the integrity boundary; input delivery does not.** §2.9's scope described a fail-closed integrity comparison for `focus-window` that the code never implemented and never intended — the comparison picks an error label, and the function is attempt-and-verify by design (A21-7, A24-16). `SetForegroundWindow` is gated by the foreground-lock heuristic, not by Mandatory Integrity Control, while UIPI covers `SendInput`/`PostMessage`, which is why a Medium→High `set-value` does fail closed across the same boundary. The contract is corrected here; no code changes.
- **A shell surface's identity is surface-kind-scoped.** `open-system-surface` returns a `w-<hwnd>` for correlation, and `snapshot --window-id` does not root it, because the window inventory deliberately excludes shell windows. Rooting it would admit a handle `list-windows` never returns and force `focus-window`, `move-window`, `resize-window` and `close-app` each to special-case it.
- **`wait --notification` keeps per-poll open/close.** §2.14's cost baseline measured `list-notifications --headed` at min 1243.5 ms, so every poll pays a full raise-read-close cycle; the documented per-poll cost is the poll floor a caller sizes a timeout against. A held-open watch session would keep the Action Center visible on the user's desktop for the wait's whole duration and would need its own teardown-on-abort path for a surface the shell can close underneath it.
- **`agent-desktop-windows` is Windows-only by design**, and the non-Windows-target build is a recorded exclusion rather than a lane. Nothing links the crate on a non-Windows target — the binary and the FFI crate take it through target-gated dependencies — and `agent-desktop-macos` not compiling off macOS is already accepted on the same terms. Building a facade so an unused check can pass is work with no consumer.
- **macOS keeps the trait default for `open-system-surface`'s `spotlight`, `dock` and `menu-bar-extras`.** The blocker is named rather than left implicit: not the absence of a macOS host — CI's macOS lane compiles and unit-tests every PR — but the absence of an **interactive** one. `native-e2e.yml` has never run, so three shell-surface implementations would be verified only against a mock, which is the shape this repository's own learning forbids.
- **The class-(b) Windows-only lifecycle envelope set, recorded unchanged:** a windowless graceful-close escalation returns `ACTION_FAILED`/`not_delivered`; a `CreateProcessW` invalid name returns `INVALID_ARGS`/`not_delivered`. The UIPI activation entry this list used to carry is **deleted** — §2.12 measured the opposite. Protected-process close refusal is shared across platforms and ships as `INVALID_ARGS`/`not_delivered`, not `PERM_DENIED`.
- **The hunk index keeps the two invariants that carry its value and retires completeness.** Every indexed hunk must name a ledger row that exists and every `CONTRADICTS` row must be backed by a hunk; both still fail the build. Completeness is retired rather than deferred, because a bijection measured against a base an entire platform phase behind counts the branch's age rather than anyone's diligence.
...
**Exit criteria:** every item in the Cross-cutting sub-phase DoD holds for the whole branch; both cross-platform contract decisions above are settled in this document rather than left to the next platform to inherit; no call site on any platform passes `AdapterError::stale_ref` anything but a ref id, pinned by a test; the live e2e CI lane is retired on the owner's decision, `windows-e2e.yml` and its static assertion deleted, the queued run flushed and the runbook rewritten to describe a retired lane, with the live suite's evidence taken from a local exclusive run and CI keeping the Windows unit, clippy, contract-gate and parity lanes that need no desktop; multi-monitor `list_displays` verification and per-display capture, genuine RDP/locked/Session-0 capture session-degradation (closing 2.0's `A10-2`), and live modern-capture pixel success (if not already discharged hosted) are each provisioned and measured or explicitly ratified as out of reach within this phase; the whole set of cross-platform contract decisions below is settled in this document rather than left for the next platform to infer, and §2.16 inherits a branch whose only remaining work is the cursor overlay and the promotion it carries.

**Est. PR size:** ~500 LOC on top of a large verification effort. Two code changes are certain to land here: the error-payload promotion (core constructors plus both adapters and a core test) and the fifteen `stale_ref` call sites, which are small but spread across core, macOS, and Windows. Either contract decision above may add a normalization change on one adapter. Everything else is review, live e2e, perf baseline, audits and the handoff to §2.16.
...
- **A timed-out clipboard read leaves a worker thread holding the Win32 clipboard open.** `read_format_bytes_on_worker` (`crates/windows/src/input/clipboard.rs`) spawns a worker that opens the real clipboard and may block inside `GetClipboardData` servicing a delay-rendered format. When the caller's deadline expires, `recv_timeout` returns `TIMEOUT` and the caller is told the read failed and is retryable — but the worker is still parked in the OS call, so its session's `Drop` cannot run and this process holds the clipboard open for the rest of the render. Every clipboard operation on the desktop then contends, including the retry the caller was just invited to make. The existing `ensure_owner_responsive()` pre-probe rules out a fully hung message pump, not a responsive-but-slow renderer, so the documented mitigation does not close this window. A thread blocked in a Win32 call cannot be killed, so the fix is not cancellation: it is refusing or serializing a subsequent clipboard operation while a worker from a previous call is still outstanding, and saying so in the envelope. No test covers the responsive-but-slow owner; `clipboard_tests.rs` covers only the fully-hung case.

- **`snapshot --surface menu` and the `surface-appeared` wait disagree about whether a menu is open.** `menu_is_open` (`crates/windows/src/system/menu_state.rs`) composes three sources: classic `GetGUIThreadInfo` menu-mode, tool-window UIA reachability, and a Chromium DOM menu. `locate_menu` (`menu_state_locate.rs`) implements only the second. An agent that waits for `surface-appeared --surface menu` against a Chromium application gets a successful wait from the third source, then asks for the surface and receives `WINDOW_NOT_FOUND`. `locate_menu`'s own doc comment claims the two resolve the same predicate and can never disagree; that claim is false and must be either made true or deleted. The same gap applies to a classic Win32 system menu with no reachable UIA element under a tool window.

- **Two fault-versus-absence collapses of the shape §2.15 fixed once.** `first_native_hwnd` (`crates/windows/src/tree/hit_test_corroborate.rs`) folds a read fault into the same `None` as a genuine absence, structurally identical to the `nearest_scroll_viewport` collapse §2.15 separated. And `EnumDisplayMonitors`'s return value is discarded in `list_displays_live` (`crates/windows/src/system/display.rs`), so a native enumeration failure returns an empty success and every downstream caller reports "no displays attached" for what was actually an API failure. Neither has a test that would fail if the distinction were removed.

- **`set-value` is advertised on a read-only range control.** `resolve_actions` (`crates/windows/src/tree/actions.rs`) advertises `SET_VALUE` unconditionally when `RangeValueAvailable` is true, with no read-only gate — unlike the adjacent `ValueAvailable` branch, which is explicitly gated on `ValueIsReadOnly` being false, because `RangeValueIsReadOnly` was never added to `TreeProperty`. A read-only progress-bar-shaped control therefore gets a ref allocated on an advertised action that fails at execution.

- **A chain rung's budget expiry discards the steps before it.** §2.15 fixed the same shape for a rung's own error: an error raised after a prior rung already delivered now carries the disposition those steps imply. `ensure_budget(deadline)?` at the top of `execute_chain`'s loop (`crates/windows/src/actions/chain.rs`) still discards its recorded steps, so a deadline that expires after a delivered-unverified write reports the deadline's disposition rather than the delivered one. It is the lesser cousin — a timeout's disposition is not the retry-safe lie the rung-error path produced — which is why it was left rather than fixed blind.

- **Key synthesis ignores the layout's shift requirement.** `vk_key_scan` (`crates/windows/src/input/keyboard_map.rs`) keeps only the low byte of `VkKeyScanW`'s result and discards the shift-state byte, so a character key is pressed with only the caller's own modifiers. On a layout where digits require Shift — AZERTY's top row, among others — `press --key 5` sends the unshifted glyph instead of `5`. The existing test exercises only the ASCII fallback branch and passes on any layout, so it cannot see this. This rig has a US layout and cannot reproduce it live; the mapping function is pure and can be tested directly.

- **One inventory read aborts on a single unresponsive window.** `list_surfaces_for_process` (`crates/windows/src/tree/surface_inventory.rs`) propagates the pump-probe failure of any one window and discards the surfaces already collected for that process — where `finish_observation` in the same crate reports an honest partial result for the analogous situation. A process owning one hung window loses its responsive windows from `list-surfaces` too.

- **Five e2e harness legs cannot fail.** `contended-focus-steal-rate` (`Acceptance.ps1`) asserts only that its trials completed, never that any won foreground, so a regression to zero-out-of-five still passes the leg whose name is the rate. `chromium-menu-attempt-bounded` (`Chromium.ps1`) records a pass regardless of what its probes observed, so the improvement it exists to catch would go unnoticed. `split-integrity-capture-recorded` (`SplitIntegrity.ps1`) computes whether pixels were produced and never gates on it, so a success envelope with a zero-byte PNG passes. `reliability-wait-enabled-delayed-button` (`Reliability.ps1`) verifies a wait solely by the command's own success flag, which is the anti-pattern this harness's contract forbids and which its own sibling leg two functions above avoids. And `FixtureControlTypeOverrideHost.cs`'s invoke provider mutates a WinForms control without marshalling to the UI thread, unlike the two sibling providers in the same corpus that guard with `InvokeRequired` — UI Automation can deliver that call off the UI thread, making the `menu-fire` leg an intermittent false pass or false fail.
...
**Release:**
- [x] Prebuilt Windows CLI binaries added to the existing `.github/workflows/release.yml` `build` matrix — as `.tar.gz` assets (`agent-desktop-v<version>-<target>.tar.gz`, single entry `agent-desktop.exe`) built natively on `windows-latest` and `windows-11-arm`, using the same sha256 + attestation pipeline Phase 1.5 ships; the Windows FFI cdylib archive remains a `.zip`
- [x] npm platform mapping extracted to one module (`npm/lib/platform.js`) consumed by postinstall and the wrapper, with released `win32-x64` / `win32-arm64` entries so `npm install -g agent-desktop` installs on Windows without changes to package shape; `check-npm-package.js` fails when the table and the release matrices disagree about targets, tarball names, or the exact asset count
- [x] The Phase 1.5 FFI cdylib for Windows (`x86_64-pc-windows-msvc`) already ships; Phase 2 added `aarch64-pc-windows-msvc` for ARM64 parity and stages the MSVC import library (`agent_desktop_ffi.dll.lib`) in the Windows archive
...
**README Update:**
- [x] Platform Support tables (README and FAQ, kept identical): Windows column states what is true — six groups work, with `list-surfaces` and `launch`-by-display-name caveated, Notifications still Planned
...
**Goal:** Bring the Windows adapter to the multi-agent cursor contract macOS now ships, and close the one session hazard that contract exposes.

**Status:** the identity model and the macOS implementation shipped on `main` in the 0.8.5 line; the Windows port ships on this branch. What remains is the session hazard in the last bullet, which neither adapter closes.

**The identity axis is settled and it is `(session, agent)`.** Agents share a session — its snapshot namespace, its trace, its manifest — and each carries a stable id supplied by the harness through the global `--agent-id` flag or `AGENT_DESKTOP_AGENT_ID`. `session start --cursor --multi-agent` turns the mode on. A per-agent `cursor-overlay enable` saves a presentation profile for that agent and starts nothing; an agent with no profile inherits the session's. Desktop UI actions require the id in this mode; observation, clipboard and administration do not.

**Both adapters derive the endpoint the same way, and the shape carries the reasons.** The endpoint name is two hash segments — the session's, then the agent's — rather than one hash of both: a single hash would make agent `a` of one session indistinguishable from agent `a` of another only by luck, and would give a session no prefix to search. The agent segment is hashed under its own domain tag, so an agent whose id equals its session's id does not reproduce the session's own segment. A caller that names no agent keeps exactly the endpoint it had before agents existed, so an ordinary single-cursor session is untouched. macOS builds a socket path this way; Windows builds a named pipe the same way, and its tests pin each of these properties by inverting them.

**A `Disable` carries no agent, deliberately, and that is what makes teardown work.** Ending a session ends all of it, and the agents that drew inside it are not the caller's to enumerate — nothing in the product is told which ids exist. So the session's live endpoints are discovered rather than derived: macOS lists its socket directory, Windows lists the pipes the OS is serving, both keeping the names under the session's prefix, and every one is told to stop. Windows attempts every endpoint even after one refuses, because returning at the first would leave the remaining cursors on screen while reporting that teardown failed.

**Windows differs from the reference in one place, and it is the platform's rule rather than a gap.** A drag is physical input; the Windows overlay is suppressed for the rest of any session that takes a headed action. So a drag instruction travels to where the drag begins and stands there, which is the whole of what can be shown truthfully, while macOS arms pointer tracking and draws a trail behind the real pointer for the length of the drag.

- **`latest_snapshot_id` is a shared mutable pointer, and under concurrency it is a defect rather than a limitation.** There is one per namespace (`crates/core/src/refs_store.rs`). Two agents in one session clobber each other's, so an agent that takes a snapshot and then issues a bare-ref action can resolve against the *other* agent's tree — a wrong-element action that reports success, which is the failure class this product refuses everywhere else. The shipped guidance already says each agent must act on qualified refs from its own snapshot; that is advice, and nothing enforces it. Decide one of: a per-agent latest pointer inside the namespace, a refusal of bare-ref actions once a session has more than one writer, or mandatory qualified refs with an error rather than a documentation sentence behind them. **This is the one item here that is neither shipped nor scoped elsewhere, and it does not depend on anything above it.**
...
- Fixture targets modeled on `AgentDeskFixture.swift` and the Windows WinForms fixture: delayed-enable, zero-bounds, duplicate-title, occlusion, disclosure
...
- **The macOS performance baseline for sub-phase 2.15's macOS-reaching changes, which that gate could not take.** §2.15 changed `agent-desktop-core` and `agent-desktop-macos` in several units whose only verification was CI's macOS lane — compile, clippy and unit tests. The Windows cost vehicle cannot measure macOS and no interactive macOS host exists for the Phase 2 line, so the baseline was recorded as a known gap rather than skipped silently. What constrained the risk in the meantime: each change was classified before it shipped, and the classification is not uniform, so it is stated per change rather than as one reassuring sentence. **Envelope-additive or provably inert:** the resolver payload promotion, which moves a constructor without changing what it constructs; the blocked-combo matcher over a three-entry constant list; and the new `live` field on a bounds read. **Genuine behaviour changes on macOS, each a branch selection on a path already being walked rather than new work per call:** the actionability verdict now returns the first blocking check that carries a terminal code rather than the first blocking check, so a disabled and policy-refused target fails fast instead of polling out its wait budget; a name-resolved ambiguous process and an absent application now answer with different codes and suggestions than before; and the shared window selector treats accessibility as a tie-break after visibility and focus rather than as a pre-filter before them. None of these adds a syscall or a traversal, which is why no latency delta is expected - but that is an argument from the shape of the change, not a measurement, and Phase 5 is where it gets measured.
...
| Phase 2 | The Windows test lane already exists as of v0.6.0; sub-phase 2.1 extends it to the adapter surface (clippy over `agent-desktop-windows`, binary-crate tests, size check); the self-hosted interactive Windows runner is registered at 2.15, the branch-final gate — the owner declined registration at 2.12 on a public-repository security ground (§2.12 Key Decisions), so 2.12's own live gate runs locally under its new `DesktopLease` instead |
